...
def generateData(X):
    ...
    for i in range(X.shape[0]):
        highFreqData[i] = addingPattern(X[i].reshape([28,28]).astype(np.float32), 1-np.real(radioMask)).reshape(784)

    lowFreqData = np.zeros(X.shape, dtype=np.float32)
    for i in range(X.shape[0]):
        lowFreqData[i] = addingPattern(X[i].reshape([28,28]).astype(np.float32), np.real(radioMask)).reshape(784)

    randomMask = mask_random_MM(p=0.5)
    randomData = np.zeros(X.shape, dtype=np.float32)
    for i in range(X.shape[0]):
        randomData[i] = addingPattern(X[i].reshape([28,28]).astype(np.float32), np.real(randomMask)).reshape(784)
...
def generateTextureData():
    ...
    Xtrain_random, Xtrain_lowFreq, Xtrain_highFreq = generateData(Xtrain)

    Xval_random, Xval_lowFreq, Xval_highFreq = generateData(Xval)

    Xtest_random, Xtest_lowFreq, Xtest_highFreq = generateData(Xtest)

    np.save('../data/MNIST/Xtrain_random', Xtrain_random)
    np.save('../data/MNIST/Xtrain_lowFreq', Xtrain_lowFreq)
    np.save('../data/MNIST/Xtrain_highFreq', Xtrain_highFreq)
