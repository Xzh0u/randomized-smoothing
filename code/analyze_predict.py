...
if __name__ == "__main__":
    ...
    gold_standard = pd.read_csv(
        "data/predict/imagenet/resnet50/noise_0.25/test/N_100000", delimiter="\t")[:450]
    for N in [100, 1000, 10000]:
        df = pd.read_csv(
            "data/predict/imagenet/resnet50/noise_0.25/test/N_{}".format(N), delimiter="\t")[:450]
        accurate = df["predict"] == gold_standard["predict"].values
        ...
        frac_incorrect_acccurate = (
            ~df["correct"] & accurate & ~abstain).mean()
        frac_incorrect_inacccurate = (
            ~df["correct"] & ~accurate & ~abstain).mean()
